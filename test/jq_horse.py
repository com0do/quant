# 克隆自聚宽文章：https://www.joinquant.com/post/67043
# 标题：三马v10.4加速版-无未来-10年124倍收益
# 作者：rbq2025

# 克隆自聚宽文章：https://www.joinquant.com/post/66791
# 标题：三马持续优化版 - v10.4
# 作者：Charlessssss
...
DEFAULT_PORTFOLIO_VALUE_PROPORTION = [0.5, 0.0, 0.5, 0.0]
STRATEGY_IDS = (1, 2, 3, 4)


def _assign_globals(params):
    """批量写入 g，避免 set_strategy_params 中重复赋值过长。"""
    for k, v in params.items():
        setattr(g, k, v)


def init_strategy_bookkeeping(context, proportions):
    """初始化四个子策略的资金、持仓与收益跟踪。"""
    if len(proportions) != len(STRATEGY_IDS):
        raise ValueError("portfolio_value_proportion length must be 4")

    g.portfolio_value_proportion = proportions
    g.starting_cash = context.portfolio.total_value
    g.stock_strategy = {}
    g.strategy_holdings = {sid: [] for sid in STRATEGY_IDS}
    g.strategy_starting_cash = {
        sid: g.starting_cash * proportions[sid - 1] for sid in STRATEGY_IDS
    }
...
def dedupe_strategy_holdings(strategy_id):
    """去重持仓列表，保持原有 list 语义。"""
    g.strategy_holdings[strategy_id] = list(set(g.strategy_holdings[strategy_id]))
...
def set_params(context):
    ...
    g.portfolio_value_proportion = DEFAULT_PORTFOLIO_VALUE_PROPORTION[:]  # 小市值/ETF轮动 (用于长回测)
    ...
    init_strategy_bookkeeping(context, g.portfolio_value_proportion)
    # 暂存一个ETF反弹的初始比例
...
def set_strategy_params(context):
    """策略1 小市值 参数"""
    _assign_globals({
        "avoid_trade_april": True,  # 是否在1、4月份避免交易小市值策略
        "huanshou_check": False,  # 放量换手检测
        "xsz_version": "v3",
        "enable_dynamic_stock_num": True,
        "xsz_stock_num": 5,
        "yesterday_HL_list": [],
        "target_list": [],
        "xsz_buy_etf": "511880.XSHG",
        # ========== 动态资金管理 ==========
        ...
        "position_ratio_min": 0.5,
        "position_ratio_max": 1.0,
        # ========== 止损检查 ==========
        "run_stoploss": True,
        "stoploss_strategy": 3,
        "stoploss_limit": 0.09,
    ...
    _assign_globals({
        "select_etf": None,  # ETF交易传递变量
        "m_days": 25,  # 动量参考天数
        "m_score": 5,  # 动量过滤分数
        "enable_stop_loss_by_cur_day": True,  # 是否开启日内止损
        "stoploss_limit_by_cur_day": -0.03,  # 当日亏损 -3%
        # 性能优化：缓存变量
        "rsrs_beta_cache": {},  # RSRS Beta值缓存字典 {stock: beta}
        "rsrs_beta_date": None,  # Beta值计算日期
    })
...
def xsz_get_stock_list_v3(context):
    ...
    q = query(
        ...
        cash_flow.subtotal_operate_cash_inflow,# 新增：经营活动现金
        ...
        cash_flow.subtotal_operate_cash_inflow > 0, # 新增：经营活动现金流入总额大于0，确保利润含金量
...
def strategy_1_buy(context):
    if not g.trading_signal:
        if g.xsz_buy_etf not in context.portfolio.positions:
            print("小市值清仓时期, 买入ETF")
            safe_etf = "511880.XSHG" if (g.DBL_control and 1 in g.dbl[-3:]) else g.xsz_buy_etf
...
def check_dbl(context, market_index='399101.XSHE', end_days=0):
    ...
    def detect_divergence():
        ...
        # 获取历史收盘价数据
        # 显式指定 end_date 为 context.previous_date，确保不包含今日未收盘数据
        grid = get_price(market_index, end_date=context.previous_date, count=rows + 10, frequency='daily', fields=['close'])
        grid = grid.dropna()
        if end_days < 0:
